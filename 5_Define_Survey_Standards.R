...
invalidhauls<-subset(HH_gov, HaulVal=="I", ) # 986 obs
write.csv(invalidhauls, "./Diagnostics/Diagnostic_data/invalid_removed_gov_29-09-2016.csv")
invalidhauls<-subset(HH_nct, HaulVal=="I", ) # 11 obs
write.csv(invalidhauls, "./Diagnostics/Diagnostic_data/invalid_removed_nct_29-09-2016.csv")
invalidhauls<-subset(HH_rot1, HaulVal=="I", ) # 18 obs
write.csv(invalidhauls, "./Diagnostics/Diagnostic_data/invalid_removed_rot_29-09-2016.csv")
invalidhauls<-subset(HH_baka, HaulVal=="I", ) # 0 obs
write.csv(invalidhauls, "./Diagnostics/Diagnostic_data/invalid_removed_baka_29-09-2016.csv")
invalidhauls<-subset(HH_beams, HaulVal=="I", ) # 430 obs
write.csv(invalidhauls, "./Diagnostics/Diagnostic_data/invalid_removed_beams_29-09-2016.csv")
...
names(HH_gov)
names(HH_rot1)
summary(as.factor(HH_rot1$Month))
HH_rot1$Month[is.na(HH_rot1$Month)]<-HH_rot1$month[is.na(HH_rot1$Month)]
HH_rot1$month<-NULL
summary(as.numeric((HH_rot1$TimeShot)))
names(HH_beams1)
HH<-rbind(HH_gov, HH_baka, HH_nct, HH_rot1, HH_beams1, fill=T)
names (HH)
# For all Surveys 
# Delete invaild hauls (this step is included above)
invalidhauls <- HH[!HH$HaulVal=='V']
write.csv(invalidhauls, "./Diagnostics/Diagnostic_data/all_invalid_hauls_29-09-2016.csv")
...
write.csv(pre1983, "./Diagnostics/Diagnostic_data/deleted_older_hauls_29-09-2016.csv")
...
list<-c('GOV','PORB','BAK','NCT','ROT','BT8','BT4A','BT7')
non_standard_gear<-subset(hauls, !Gear%in%list,)
hauls<-subset(hauls, Gear%in%list, )
summary(hauls$Gear)
write.csv(non_standard_gear, "./Diagnostics/Diagnostic_data/deleted_non_standard_gear_hauls_29-09-2016.csv")
...
write.csv(smallhauls, "./Diagnostics/Diagnostic_data/deleted_too_short_hauls_29-09-2016.csv")
largehauls<- subset(hauls, HaulDur>67 ,)
hauls<- subset(hauls, HaulDur<67 , )
write.csv(largehauls, "./Diagnostics/Diagnostic_data/deleted_too_long_gear_hauls_29-09-2016.csv")
...
write.csv(hauls, "./Diagnostics/Diagnostic_data/Working_HH_file_28_07_2016.csv")
...
HL1<-subset(HL, New_UniqueID%in%check,)
length(unique(HL1$New_UniqueID))
# this gives 172mismatched hauls now.
list<-unique(HL1$New_UniqueID)
hauls1<-subset(hauls, New_UniqueID%in%list,)
length(unique(HL1$New_UniqueID))
length(unique(hauls1$New_UniqueID))
# hopefully this solves some problems of the not so unique ids :)

write.csv(hauls, "./Diagnostics/Diagnostic_data/Working_HH_file_29-09-2016.csv")
write.csv(HL, "./Diagnostics/Diagnostic_data/Working_HL_file_29-09-2016.csv")

write.csv(hauls1, "./Diagnostics/Diagnostic_data/Matching_HH_file_29-09-2016.csv")
write.csv(HL1, "./Diagnostics/Diagnostic_data/Matching_HL_file_29-09-2016.csv")
